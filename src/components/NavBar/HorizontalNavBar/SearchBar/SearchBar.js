...
const SearchBar = React.memo(({ className, query, active }) => {
    ...
    const debouncedUpdateUrl = React.useCallback(
        debounce((value) => {
            if (value) {
                window.location.hash = `/search?search=${encodeURIComponent(
                    value
                )}`;
            } else {
    ...
    const searchHistoryOnClose = React.useCallback(
        (event) => {
            if (
                historyOpen &&
    ...
    }, [createTorrentFromMagnet, debouncedUpdateUrl]);

    const queryInputOnSubmit = React.useCallback((event) => {
        event.preventDefault();
        const searchValue = `/search?search=${encodeURIComponent(
            event.target.value
        )}`;
    ...
    const updateLocalSearchDebounced = React.useCallback(
        debounce((query) => {
                ...
                <div className={styles["menu-container"]}>
                    ...
                    {localSearch?.items?.length ? (
                        <div className={styles["items"]}>
                            ...
                            {localSearch.items.map(
                                ({ query, deepLinks }, index) => (
                                    <Button
                                        key={index}
                                        className={styles["item"]}
                                        href={deepLinks.search}
